...
class ASR(sb.core.Brain):
    ...
    def fit_batch(self, batch):
        ...
        if self.auto_mix_prec:
            ...
            times = append_batch_to_group_dict2(batch, feats, p_ctc, losses, self) # self.attribute, reservoir, asr
            
            if not self.reservoir.init_phase: #and (self.n_diff is not None):
                if times == self.n_diff:
                    self.n_diff = None
                elif self.n_diff is not None: # times < n_diff                    
                    self.n_diff -= times
