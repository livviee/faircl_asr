...
def next_not_none_attribute_batch(times, attribute, train_loader):
    while True:
        ...
        if(next_group != ''):
            return times, next_batch, train_loader
...
def entropy_based_data_selection(asr, size, attribute, train_loader, csv_file):
    ...
    #asr.reservoir = reservoir
    #asr.add_reservoir_to_recoverables()
    ...
    times, next_batch, train_loader = next_not_none_attribute_batch(times, attribute, train_loader)
    ...
    while next_batch is not None:
        ...
        if (n_diff == 0):
            min_dist_ids, min_dist_objects = find_min_dist_sample_in_majority_group(reservoir, 1)
        else:
            min_dist_ids, min_dist_objects = find_min_dist_sample_in_majority_group(reservoir, n_diff)
